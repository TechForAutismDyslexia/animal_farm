...
function Page22() {
  ...
  const checkSuccess = () => {
    if (dragItemRef.current) {
      ...
      const cowBesidedog = cowRect.right >= dogRect.left && cowRect.left <= dogRect.right;
  
      // Bottom alignment condition with a threshold of 10px
      const bottomAlignmentThreshold = 30;
      const cowBottomAlignedWithdog = Math.abs(cowRect.bottom - dogRect.bottom) <= bottomAlignmentThreshold;
  
      // If both conditions are met, it's a success
      if (cowBesidedog && cowBottomAlignedWithdog) {
  ...
  useEffect(() => {
    ...
    readOutLoud('The dog has stopped in front of the cow.');
  ...
  return (
    <div ref={pixiContainerRef} style={{ position: 'relative', width: '100%', height: '100%' }}>
      <span
        ...
        onClick={() => readOutLoud("The dog has stopped in front of the cow.")}
      >
        <img src="/images/sound.webp" alt="sound" width={'50px'} />
      </span>
      <p className="text">
        The dog has stopped in front of the cow.
      </p>
      <img
        className="images"
        id="dog"
